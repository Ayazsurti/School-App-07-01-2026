...
const Attendance: React.FC<AttendanceProps> = ({ user }) => {
  ...
  const [mode, setMode] = useState<AttendanceMode>(isStudent ? 'STUDENTS' : 'STUDENTS');
  const [selectedClass, setSelectedClass] = useState(isStudent ? (user as any).class || '10th' : '1st');
  const [selectedSection, setSelectedSection] = useState(isStudent ? (user as any).section || 'A' : 'A');
  ...
  // Mock Student Attendance Data for Display
  ...
  const currentList = useMemo(() => {
    if (isStudent) return [(MOCK_STUDENTS as any[]).find(s => s.id === user.id) || MOCK_STUDENTS[0]];
  ...
  }, [mode, selectedClass, selectedSection, isStudent, user.id]);
  ...
  useEffect(() => {
    if (isStudent) {
        // Mocking current day as present for student view
        setAttendance({ [user.id]: 'PRESENT' });
    } else {
        setAttendance({});
    }
  }, [mode, selectedClass, selectedSection, selectedDate, isStudent, user.id]);

  const handleQuickMark = (e: React.FormEvent) => {
    e.preventDefault();
    if (isStudent) return; // Restricted
  ...
  const toggleStatus = (id: string, status: AttendanceStatus) => {
    if (isStudent) return; // Restricted: Students cannot change attendance
    setAttendance(prev => ({
      ...prev,
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight uppercase leading-tight">
            {isStudent ? 'My Attendance Profile' : 'Attendance Registry'}
          </h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium text-lg">
            {isStudent ? 'View your verified academic presence logs.' : `Real-time tracking and counting for ${mode.toLowerCase()}.`}
          </p>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          {isStudent ? (
            <>
              {[
                { label: 'Total Working Days', value: studentStats.totalDays, color: 'text-slate-400', icon: <Calendar size={16}/>, bg: 'bg-white dark:bg-slate-900' },
                { label: 'Days Present', value: studentStats.present, color: 'text-emerald-600', icon: <UserCheck size={16}/>, bg: 'bg-emerald-50/30 dark:bg-emerald-900/10' },
                { label: 'Days Absent', value: studentStats.absent, color: 'text-rose-600', icon: <UserX size={16}/>, bg: 'bg-rose-50/30 dark:bg-rose-900/10' },
                { label: 'Days Late', value: studentStats.late, color: 'text-amber-600', icon: <Clock size={16}/>, bg: 'bg-amber-50/30 dark:bg-amber-900/10' },
                { label: 'Current Score', value: `${studentStats.percentage}%`, color: 'text-indigo-600', icon: <Trophy size={16}/>, bg: 'bg-indigo-50/30 dark:bg-indigo-900/10' },
              ].map((s, idx) => (
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden relative">
            <div className="flex items-center justify-between mb-8 relative z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-black text-slate-900 dark:text-white text-sm uppercase tracking-tight">Presence Calendar</h3>
            ...
            <div className="grid grid-cols-7 gap-1">
              {daysInMonth.map((day, idx) => (
                <div key={idx} className="aspect-square flex items-center justify-center p-0.5">
                  {day ? (
                    <button 
                      onClick={() => !isStudent && setSelectedDate(day)}
                      disabled={isStudent}
            ...
            <div className="mt-6 pt-6 border-t border-slate-50 dark:border-slate-800 text-center">
               <span className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">
                 {isStudent ? 'Personal Archive Access' : `Logging for: ${selectedDate.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}`}
               </span>
            </div>
          </div>
          ...
          {!isStudent ? (
            ...
            <div className="bg-indigo-600 rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden group">
               ...
               <Lock className="text-indigo-200 mb-6" size={40} />
               <h3 className="text-xl font-black uppercase tracking-tight mb-2">Registry Integrity</h3>
               <p className="text-indigo-100 text-sm font-medium leading-relaxed">Attendance records are read-only for students. Any discrepancies must be reported to the class teacher within 48 hours of logging.</p>
               <button className="mt-8 px-6 py-3 bg-white/10 hover:bg-white/20 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Request Correction</button>
            </div>
          )}
        </div>

        {/* Right Column: Registry Terminal / List */}
        <div className="lg:col-span-8">
          <div className="bg-white dark:bg-slate-900 rounded-[3.5rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
            <div className="px-10 py-8 border-b border-slate-50 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                 <h3 className="font-black text-slate-900 dark:text-white text-2xl uppercase tracking-tight">
                    {isStudent ? 'My Presence Logs' : 'Registry Terminal'}
                 </h3>
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                    {isStudent ? `Academic Session 2026-27 • Current Status` : `${mode === 'STUDENTS' ? `Std ${selectedClass}-${selectedSection}` : 'Faculty Roster'} • Accounted: ${stats.markedCount}/${stats.total}`}
                 </p>
              </div>
              {!isStudent && (
              ...
              <table className="w-full min-w-[700px]">
                <thead>
                  <tr className="bg-slate-50/50 dark:bg-slate-800/50 text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] border-b border-slate-100 dark:border-slate-800">
                    ...
                    <th className="px-10 py-6 text-right">{isStudent ? 'Timeline' : 'Commit Entry'}</th>
                  </tr>
                </thead>
                        ...
                        <td className="px-10 py-8">
                          <div className="flex items-center gap-5">
                            ...
                            <div className="min-w-0">
                               ...
                               <p className="text-[10px] text-indigo-400 font-bold uppercase tracking-widest mt-0.5">Verified Profile</p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-10 py-8 text-right">
                          {isStudent ? (
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                               Logged: {selectedDate.toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}
                            </span>
                          ) : (
