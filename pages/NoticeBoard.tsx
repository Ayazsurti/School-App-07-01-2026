...
const NoticeBoard: React.FC<NoticeBoardProps> = ({ user }) => {
  ...
  // Helper: Convert Base64 to safe Blob URL for Chrome
  ...
  // Sync effect to handle Chrome's blocking of Data URLs
  useEffect(() => {
    if (showViewModal && selectedNoticeForView?.attachments) {
      ...
      // Cleanup URLs from memory when closing
      return () => {
        urls.forEach(url => {
  ...
  useEffect(() => {
    try {
      ...
      setErrorMsg("Storage Quota Exceeded: Please delete old notices to free up space.");
    }
  }, [notices]);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type !== 'application/pdf') {
      alert("Invalid Format: Only PDF documents are allowed.");
      return;
    }
    if (file.size > 2 * 1024 * 1024) {
      alert("File Too Large: PDF must be under 2MB for stability.");
      return;
    }
  ...
  const handlePost = () => {
    ...
    if (editingNoticeId) {
      ...
      triggerSuccess('Broadcast Published');
    }
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 relative">
      ...
      {errorMsg && (
        <div className="fixed bottom-8 left-8 right-8 z-[1100] animate-in slide-in-from-bottom-4">
           <div className="bg-rose-600 text-white p-6 rounded-[2rem] shadow-2xl flex items-center justify-between border border-rose-500">
              <div className="flex items-center gap-4">
                 <AlertCircle size={24} className="animate-pulse" />
                 <p className="font-bold text-sm">{errorMsg}</p>
              </div>
              <button onClick={() => setErrorMsg(null)} className="p-2 hover:bg-white/10 rounded-full transition-colors"><X size={20}/></button>
           </div>
        </div>
      )}
      ...
      {showSuccess && (
        <div className="fixed top-24 right-8 z-[1000] animate-in slide-in-from-right-8 duration-500">
           <div className="bg-emerald-600 text-white px-8 py-5 rounded-[2.5rem] shadow-2xl flex items-center gap-4 border border-emerald-500/50 backdrop-blur-xl">
              <CheckCircle2 size={24} strokeWidth={3} className="text-white" />
              <div>
                 <p className="font-black text-xs uppercase tracking-[0.2em]">Registry Updated</p>
      ...
      {/* PDF VIEWER TERMINAL (FIXED WITH BLOB URLS TO PREVENT CHROME BLOCKING) */}
      {showViewModal && selectedNoticeForView && (
              ...
              <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/30">
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                       ...
                       <p className="text-slate-400 dark:text-slate-500 text-[10px] font-bold uppercase tracking-widest mt-1">Official Broadcast • Secure Blob Tunnel</p>
                    </div>
                 </div>
                 ...
                 <div className="mb-10">
                    <div className="flex items-center gap-3 mb-6">
                       ...
                       <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                          <Clock size={14} className="text-indigo-500" /> Issued: {selectedNoticeForView.date}
                       </span>
                    </div>
                      ...
                      <div className="space-y-8 pt-10 border-t border-slate-100 dark:border-slate-800">
                         <div className="flex items-center justify-between">
                            <h4 className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-[0.3em] flex items-center gap-3">
                               <FileText size={18} className="text-indigo-600" /> Verified PDF Assets
                            </h4>
                            {isStudent && (
                              <div className="flex items-center gap-2 px-4 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-full border border-slate-200 dark:border-slate-700">
                                 ...
                                 <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">View Only Mode</span>
                              </div>
                            )}
                         </div>
                                 ...
                                 <div className="flex-1 relative bg-white dark:bg-slate-800 flex items-center justify-center overflow-hidden">
                                    {viewableUrls[idx] ? (
                                      ...
                                      <div className="flex flex-col items-center gap-4 text-slate-300 animate-pulse">
                                         ...
                                         <p className="font-black uppercase text-xs">Generating safe view...</p>
                                      </div>
                                    )}
                                 </div>
                                 <div className="p-8 flex items-center justify-between bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800">
                                    <div className="flex items-center gap-4">
                                       ...
                                       <div className="min-w-0">
                                          ...
                                          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Authorized PDF Content</p>
                                       </div>
                                    </div>
                                    ...
                                    {!isStudent ? (
                                      <button 
                                        ...
                                        className="flex items-center gap-2 px-10 py-4 bg-emerald-600 text-white font-black text-[11px] rounded-2xl uppercase tracking-widest shadow-xl hover:bg-emerald-700 transition-all active:scale-95"
                                      >
      ...
      {deleteConfirmationId && (
        <div className="fixed inset-0 z-[750] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in">
           <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-12 max-sm w-full shadow-2xl text-center border border-rose-100 dark:border-rose-900/50 animate-in zoom-in-95">
              ...
              <p className="text-slate-500 dark:text-slate-400 mb-10 font-medium text-xs leading-relaxed uppercase tracking-widest">Erase this broadcast from the permanent student portal registry.</p>
              <div className="grid grid-cols-2 gap-4">
                 <button onClick={() => setDeleteConfirmationId(null)} className="py-5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-black rounded-3xl uppercase text-[10px] tracking-widest hover:bg-slate-200 transition-all">Cancel</button>
                 <button onClick={confirmDeleteNotice} className="py-5 bg-rose-600 text-white font-black rounded-3xl shadow-xl hover:bg-rose-700 transition-all uppercase text-[10px] tracking-widest">Purge Now</button>
              </div>
           </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium text-lg">
            {isStudent ? 'Official academic bulletins and safe-view material.' : 'Post announcements and secure PDF documents to students.'}
          </p>
        </div>
        {canManage && (
          <button onClick={() => { setShowForm(true); setEditingNoticeId(null); setNewNotice({ title: '', content: '', category: 'GENERAL', attachments: [] }); }} className="px-10 py-5 bg-indigo-600 text-white font-black rounded-3xl shadow-2xl flex items-center gap-4 hover:bg-indigo-700 transition-all uppercase text-xs tracking-widest">
            <Plus size={20} strokeWidth={3} /> Post Bulletin
          </button>
        )}
      </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] shadow-sm border border-slate-100 dark:border-slate-800">
              <h3 className="font-black text-slate-800 dark:text-white mb-6 flex items-center gap-3 text-sm uppercase tracking-[0.2em]">
                <Tag size={18} className="text-indigo-500" /> Filter Archive
              </h3>
              <div className="space-y-2">
                {['All', ...categories].map(cat => (
                  <button key={cat} onClick={() => setSelectedCategory(cat)} className={`w-full text-left px-6 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${selectedCategory === cat ? 'bg-indigo-600 text-white shadow-xl' : 'text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>{cat === 'All' ? 'All Bulletins' : cat}</button>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-8">
            {filteredNotices.length > 0 ? (
                ...
                <div key={notice.id} onClick={() => { setSelectedNoticeForView(notice); setShowViewModal(true); }} className="bg-white dark:bg-slate-900 p-10 lg:p-14 rounded-[4rem] shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-2xl transition-all cursor-pointer relative group overflow-hidden hover:-translate-y-1">
                  ...
                  <div className="flex items-center justify-between pt-10 border-t border-slate-50 dark:border-slate-800">
                     <div className="flex items-center gap-3 text-indigo-600 dark:text-indigo-400 font-black text-[11px] uppercase tracking-[0.3em] group-hover:translate-x-4 transition-all">
                        <Eye size={20} /> Initialize Safe View <ChevronRight size={20} />
                     </div>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-[5rem] py-60 text-center border-4 border-dashed border-slate-100 dark:border-slate-800 flex flex-col items-center">
                 ...
                 <h3 className="text-3xl font-black text-slate-800 dark:text-white uppercase tracking-tighter">Vault Empty</h3>
                 <p className="text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest text-[10px] mt-2">Adjust search terms or clear filters.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[700] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in">
           <div className="bg-white dark:bg-slate-900 rounded-[4rem] p-1 shadow-2xl max-w-3xl w-full max-h-[95vh] overflow-hidden flex flex-col border border-slate-100 dark:border-slate-800">
              <div className="p-10 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/30">
                 <div>
                   <h3 className="text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tight">{editingNoticeId ? 'Modify Record' : 'Publish Broadcast'}</h3>
                   <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Institutional Data Synchronization</p>
                 </div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-12 space-y-10">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                       <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-3 ml-1">Identity (Title)</label>
                    ...
                    <div>
                       <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-3 ml-1">Classification Pool</label>
                 ...
                 <div className="space-y-6">
                    ...
                    >
                       ...
                       <span className="text-[10px] uppercase tracking-[0.3em]">Initialize Secure PDF Sync</span>
                    </button>
                    <input type="file" ref={fileInputRef} onChange={handleFileUpload} className="hidden" accept=".pdf" />

                    <div className="grid grid-cols-1 gap-4">
                       {newNotice.attachments.map((file, idx) => (
                         <div key={idx} className="p-6 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-3xl flex items-center justify-between shadow-sm">
                            <div className="flex items-center gap-5 min-w-0">
                               ...
                               <div className="min-w-0">
                                  ...
                                  <span className="text-[9px] font-bold text-emerald-600 uppercase tracking-widest">Document Ready</span>
                               </div>
                            </div>
              ...
              <div className="p-12 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30">
                 <button onClick={handlePost} className="w-full py-7 bg-indigo-600 text-white font-black rounded-3xl shadow-2xl shadow-indigo-100 hover:bg-indigo-700 transition-all uppercase text-sm tracking-[0.3em] flex items-center justify-center gap-4">
                    <ShieldCheck size={20} strokeWidth={2.5} /> Finalize Official Broadcast
                 </button>
              </div>
           </div>
        </div>
