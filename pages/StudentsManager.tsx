...
const StudentsManager: React.FC<StudentsManagerProps> = ({ user }) => {
  ...
  const fetchStudents = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setStudents(saved ? JSON.parse(saved) : MOCK_STUDENTS as Student[]);
      } else if (data) {
        // Map Supabase column names to Student interface if they differ
        const mapped: Student[] = data.map((d: any) => ({
          ...
          uidNumber: d.uidNumber || d.uid_number,
          grNumber: d.grNumber || d.gr_number,
          residenceAddress: d.residenceAddress || d.residence_address,
          fatherName: d.fatherName || d.father_name,
          motherName: d.motherName || d.mother_name,
          fatherMobile: d.fatherMobile || d.father_mobile,
          motherMobile: d.motherMobile || d.mother_mobile,
          profileImage: d.profileImage || d.profile_image
        }));
        setStudents(mapped);
        localStorage.setItem('school_students_db', JSON.stringify(mapped));
      }
    } catch (err) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(`Backend Sync Error: ${err.message}. Saving locally instead.`);
      // Local fallback
      if (editingStudent) {
        setStudents(prev => prev.map(s => s.id === editingStudent.id ? { ...s, ...payload } as Student : s));
      } else {
        setStudents(prev => [{ ...payload, id: Math.random().toString() } as Student, ...prev]);
      }
      setShowModal(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      createAuditLog(user, 'DELETE', 'Registry', `Purged: "${target?.fullName}"`);
      await fetchStudents();
      setDeleteId(null);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in duration-500 relative">
      ...
      {showSuccess && (
        <div className="fixed top-24 right-8 z-[1000] animate-in slide-in-from-right-8 duration-500">
           <div className="bg-emerald-600 text-white px-8 py-5 rounded-[2rem] shadow-2xl flex items-center gap-4 border border-emerald-500/50 backdrop-blur-xl">
              ...
              <div>
                 <p className="font-black text-xs uppercase tracking-[0.2em]">Live Sync Success</p>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-[700] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
           <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-10 max-sm w-full shadow-2xl text-center border border-slate-100 dark:border-slate-800">
              ...
              <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-2 uppercase tracking-tight">Purge Student?</h3>
              <p className="text-slate-500 dark:text-slate-400 font-medium mb-8">This will permanently erase all records from Supabase.</p>
      ...
      {/* Profile PDF Viewer Terminal - Remains Branded and Detailed */}
      {viewingStudentPdf && (
            ...
            <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/30 dark:bg-slate-800/30 sticky top-0 z-20 backdrop-blur-md">
              <div className="flex items-center gap-4">
                ...
                <div>
                   ...
                   <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mt-1">Backend: Supabase Cloud Connected</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-10 custom-scrollbar space-y-12">
              <div className="flex flex-col md:flex-row gap-12">
                 <div className="flex flex-col items-center gap-6">
                    ...
                    <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase text-center tracking-widest">Enrollment Photo</p>
                 </div>
                 ...
                 <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                       ...
                       <div className="relative">
                          <QrCode className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-600" size={18} />
              ...
              <div className="space-y-8">
                 <div className="flex items-center gap-3 border-l-4 border-indigo-600 dark:border-indigo-500 pl-4 py-1">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                    <div className="space-y-6">
                       <div>
                          <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 ml-1">Father's Full Name</label>
                          <div className="relative">
                             <UserIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-600" size={18} />
                             <input type="text" value={formData.fatherName} onChange={e => setFormData({...formData, fatherName: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl pl-12 pr-4 py-4 font-bold text-slate-700 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 shadow-inner" placeholder="Father's name" />
                          </div>
                       </div>
                       <div>
                          <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 ml-1">Father's Mobile No.</label>
                          <div className="relative">
                    ...
                    <div className="space-y-6">
                       <div>
                          <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 ml-1">Mother's Full Name</label>
                          <div className="relative">
                             <UserIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-600" size={18} />
                       ...
                       <div>
                          <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 ml-1">Mother's Mobile No.</label>
                          <div className="relative">
                             <Smartphone className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-600" size={18} />
              ...
              <div className="space-y-8">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div>
                       ...
                       <div className="relative">
                          <CreditCard className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-600" size={18} />
              ...
              <div>
                 ...
                 <div className="relative">
                    <MapPin className="absolute left-4 top-6 text-slate-300 dark:text-slate-600" size={18} />
              ...
              <div className="grid grid-cols-2 gap-6 pt-6 border-t border-slate-100 dark:border-slate-800">
                ...
                <button type="submit" className="py-5 bg-indigo-600 text-white font-black text-xs uppercase tracking-[0.2em] rounded-3xl shadow-2xl shadow-indigo-100 dark:shadow-indigo-900/20 hover:bg-indigo-700 transition-all">Synchronize Cloud</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 no-print">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium text-lg">Institutional database tracking identifiers via Supabase Backend.</p>
        </div>
        ...
        <div className="flex gap-4 w-full xl:w-auto overflow-x-auto no-scrollbar">
          <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800/50 p-2 rounded-2xl border border-slate-100 dark:border-slate-800 flex-shrink-0">
             ...
             <div className="flex gap-1">
               {['All', ...ALL_CLASSES.slice(0, 5)].map(c => (
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[3rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden no-print min-h-[400px] relative">
        {isLoading ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm z-10">
            ...
            <p className="text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Authenticating Ledger...</p>
          </div>
        ) : (
                      ...
                      <td className="px-10 py-6">
                        <div className="flex items-center gap-5">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-indigo-500 dark:text-indigo-400 font-black mt-1 uppercase tracking-widest">{student.email}</p>
                          </div>
                        </div>
                      </td>
